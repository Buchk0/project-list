...
function sendTask(event) {
    ...
    if (!task) {
        return alert('Поле не має бути пустим');
    }
...
function doneTask(event) {
    ...
    if (index === -1) {
        return alert('Такої задачі не знайдено!')
    }
...
function pinnedTask(event) {
    ...
    if (!arrayTasksLS[index].pinned && arrayTasksLS[index].done) {
        return alert('Щоб закріпити задачу, спочатк употрібно забрати позначку про виконання!')
    }
...
function saveEditedTask(task) {
    ...
    if (editedTaskIndex !== -1) {
        ...
        alert('Такої задачі не знайдено!')
    }
    resetSendForm();
}
...
function resetSendForm() {
    ...
    buttonSendForm.textContent = 'Добавити';
    form.reset();
}
